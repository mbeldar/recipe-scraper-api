...
def normalize_ingredient(ingredient_str):
    ...
    try:
        ...
        if isinstance(parsed, dict):
            ...
            logger.debug("_normalize_ingredient(dict) -> %r %r %r", quantity, unit, name)
        ...
        if hasattr(parsed, 'amount') and parsed.amount:
            try:
                ...
                if unit_obj:
                    unit = clean_unit_string(unit_obj)
            except Exception:
        ...
        logger.debug("_normalize_ingredient(object) -> %r %r %r", quantity, unit, name)
        return {'quantity': quantity, 'unit': unit, 'name': name}
